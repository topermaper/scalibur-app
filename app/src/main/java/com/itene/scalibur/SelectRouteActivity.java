...
public class SelectRouteActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    private void getRoutesConfirmed() {
        try {
            ...
            // Get driver's routes, running or ready
            String url = String.format(Config.API_GET_ROUTES + String.format("?status=RUNNING,READY&driver_id=%d", user.getId()));

            VolleyUtils.GET_JSON_ARRAY(this, url, api_token, new VolleyUtils.VolleyJsonArrayResponseListener() {
                @Override
                public void onError(String message) {
                    Log.e("API", "No se puede leer en la plataforma, error: " + message);
                    Toast.makeText(SelectRouteActivity.this, String.format("There was an error trying to get routes", user.getName()), Toast.LENGTH_LONG).show();
                }
                ...
                public void onResponse(JSONArray response) {
                    try {
                        ...
                        if (mRouteList.size() > 0) {
                            ...
                            Toast.makeText(SelectRouteActivity.this, String.format("Hi %s, there are no routes assigned to you", user.getName()), Toast.LENGTH_LONG).show();
                        }
